    ...
    public async Task<ActionResult> Register(RegisterViewModel model)
    {
      if (ModelState.IsValid)
      {
        ...
        if (result.Succeeded)
        {
          ...
          var currentUser = ApplicationDbContext.Users.FirstOrDefault(x => x.UserName == HttpContext.User.Identity.Name); 
          ...
          else if (model.UserRoles == "Faculty")
          {
            Faculty faculty = new Faculty();
            faculty.Email = user.Email;
            faculty.User = currentUser;
            ApplicationDbContext.Faculties.Add(faculty);
            ApplicationDbContext.SaveChanges();
            return RedirectToAction("Index", "Faculties");
          }
          else if (model.UserRoles == "Admin")
          {
